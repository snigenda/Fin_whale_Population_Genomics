...
def asym_mig_size(params, ns, pts):
    ...
    phi = Integration.two_pops(phi, xx, Td, nu1=nu1, nu2=nu2a,m12=m12, m21=m21) #Divergence time
    
    phi = Integration.two_pops(phi, xx, Tc, nu1=nu1, nu2=nu2b,m12=m12, m21=m21) #Change in the Gulf
...
param_names=("Ta","nua","Td","nu1","nu2a","nu2b","Tc","m12","m21")

lower_bound = [1e-4, 1e-9, 1e-5, 1e-5,1e-5, 1e-5, 1e-5,1e-9, 1e-5]
upper_bound = [10, 20, 10,10, 10,5,10,10,700] # 20 as upper bound on mig rec by dadi
p0 = [0.113,1.54,0.03,1.1,0.0071,0.0001,0.001,2.5,110] # initial parameters
...
nu2a_scaled_dip=popt[4]*Nanc
nu2b_scaled_dip=popt[5]*Nanc

Ta_scaled_gen=popt[0]*2*Nanc
Td_scaled_gen=popt[2]*2*Nanc
Tc_scaled_gen=popt[6]*2*Nanc

m12_fraction=popt[7]/(2*Nanc)
m21_fraction=popt[8]/(2*Nanc)


scaled_param_names=("Nanc","Ta","nua","Td","nu1","nu2a", "nu2b","Tc", "migrationFraction12", "migrationFraction21")

scaled_popt=(Nanc,Ta_scaled_gen,nua_scaled_dip,Td_scaled_gen,nu1_scaled_dip,nu2a_scaled_dip,nu2b_scaled_dip,Tc_scaled_gen,m12_fraction,m21_fraction)
