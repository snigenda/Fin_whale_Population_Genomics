...
# load data --------
gtlist = readRDS(file = paste0(indir, 'HML_all50_snpEff_matching_Minke_GTtable_Seg_PASSm6_20210422.rds'))

# main --------
rxyoutputlist = lapply(prefixlist, function(xx) {
    print(paste0(date(), ': Calculating Lxy on ', xx, ' from ', indir, 'HML_all50_snpEff_matching_Minke_GTtable_Seg_PASSm6_20210422.rds'))
    gtseg = gtlist[[xx]]
    lxydt = get_lxy(gtseg, xpop = 'GOC', ypop = 'ENP')
    saveRDS(lxydt, file = paste0(outdir1, xx, '_all50_snpEff_matching_Minke_Lxy_xGOCyENP_PASSm6_', today, '.rds'))
    ...
    saveRDS(rxy_jk[[3]], file = paste0(outdir2, xx, '_all50_snpEff_matching_Minke_RxyJK1000_xGOCyENP_PASSm6_', today, '.rds'))
    return(rxy_jk)
})
...
saveRDS(rxy_summary, file = paste0(outdir3, 'Summary_all50_snpEff_matching_Minke_Rxy_xGOCyENP_PASSm6_', today, '.rds'))
saveRDS(r2xy_summary, file = paste0(outdir3, 'Summary_all50_snpEff_matching_Minke_R2xy_xGOCyENP_PASSm6_', today, '.rds'))

# output the summary jackknife ========
jk_forplot = dplyr::bind_rows(lapply(rxyoutputlist, function(x) {x[[3]]}))
saveRDS(jk_forplot, file = paste0(outdir3, 'Summary_all50_snpEff_matching_Minke_1000JK_xGOCyENP_PASSm6_', today, '.rds'))
...
ggsave(filename = paste0('QC_Rxy_all50_snpEff_matching_Minke_1000JK_xGOCyENP_PASSm6.pdf'),
