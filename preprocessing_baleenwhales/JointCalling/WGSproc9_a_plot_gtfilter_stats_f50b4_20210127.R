...
for (ii in 1:get_contigs(ref)) {
    ...
    gtfttemp = read.csv(gtfilterfile, stringsAsFactors = F,
                        colClasses = c("character", rep("numeric", times = nsample))) %>%
...
totalgt = colSums(apply(gtfiltertally[,2:(nsample+1)], 2, as.numeric))
unique(totalgt) # 2324429717 (the total genotype called available) (2324429847 is the total length of reference sequence)
